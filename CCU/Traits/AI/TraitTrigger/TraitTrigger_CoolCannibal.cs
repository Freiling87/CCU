    ...
    public class TraitTrigger_CoolCannibal : CustomTrait
    {
        ...
        public static void Setup()
        {
            RogueLibs.CreateCustomTrait<TraitTrigger_CoolCannibal>()
                ...
                {
                    [LanguageCode.English] = String.Format("This NPC's behaviors will react to the player's Cool with Cannibals trait.\n\n<color=green>{0}</color>, <color=green>{1}</color>: Will not target the player if they have Cool with Cannibals.\n\n<color=green>{2}</color>: Will not sell wares unless player has Cool With Cannibals.", CTrait.AI_RoamBehavior_HideInBush, CTrait.AI_RoamBehavior_HideInManhole, CTrait.AI_Vendor_Cannibal),
