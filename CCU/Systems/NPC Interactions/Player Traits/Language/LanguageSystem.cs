...
namespace CCU.Systems.Language
{
	public class LanguageSystem : ISetupAgentStats
	{
		...
		// TODO: Localize names when mentioned in dialogue
		...
		private static void Setup()
		{
			...
			// TODO: I think the original method has been patched to the point that this interaction no longer gets through.
			// It might as well just go into the patches since it isn't technically an interaction, and I'm not sure how to do priority.
			...
			//	{
			//		logger.LogDebug("Language Interaction"); // Fires

			//		// h.SetSideEffect(m => SayGibberish(m.Object)); // dw

			//		//h.AddImplicitButton("Interact", m =>
			//		//{
			//		//	logger.LogDebug("Language Interaction 2"); // Doesn't fire
			//		//	SayGibberish(agent);
		...
		public static bool HaveSharedLanguage(Agent agent, Agent otherAgent) =>
			...
			|| KnownLanguages(agent).Intersect(KnownLanguages(otherAgent)).ToList().Any();

		public static string LanguageDialogueName(Agent agent)
		{
			if (agent.agentName is VanillaAgents.CustomCharacter)
			{
				List<string> spokenLangs = KnownLanguages(agent);
		...
		public static List<string> KnownLanguages(Agent agent)
		{
	...
	public class P_Agent_Language
	{
		...
		// Also what the fuck is with the method name
	...
	public static class P_InteractionHelper_Language
	{
		...
		{
			List<string> langs = LanguageSystem.KnownLanguages(otherAgent);
