						...
						if (!___agent.dead && !___agent.frozen && !___agent.hasGettingBitByAgent && !___agent.chargingForward && !___agent.oma.hidden && !___agent.oma.mindControlled)
						{
							...
							if (___agent.losCheckAtIntervals && ___agent.isPlayer == 0)
							{
								...
								if (___agent.losCheckAtIntervalsTime > 8)
								{
									if (___agent.agentName == "Hobo")
									{
										...
										if (!___agent.hasEmployer)
										{
											...
											for (int n = 0; n < moneyList.Count; n++)
											{
												...
												if (!item2.fellInHole && item2.curTileData.prison == ___agent.curTileData.prison && !item2.dontStealFromGround && (___agent.curTileData.prison <= 0 || ___agent.curTileData.chunkID == item2.curTileData.chunkID) && !__instance.gc.tileInfo.DifferentLockdownZones(___agent.curTileData, item2.curTileData) && ___agent.curPosX - 5f < item2.curPosition.x && ___agent.curPosX + 5f > item2.curPosition.x && ___agent.curPosY - 5f < item2.curPosition.y && ___agent.curPosY + 5f > item2.curPosition.y && ___agent.movement.HasLOSObjectNormal(item2))
												{
									...
									else if (___agent.agentName == "Cannibal")
									{
										___agent.losCheckAtIntervalsTime = 0;
										if (!___agent.hasEmployer || ___agent.health <= 15f)
										{
											...
											{
												Agent agent5 = deadAgentList[num4];
												try
												{
													if (agent5.dead && !agent5.resurrect && !agent5.ghost && !agent5.disappeared && !agent5.inhuman && !agent5.cantCannibalize && !agent5.hasGettingBitByAgent && ___agent.prisoner == agent5.prisoner && ___agent.slaveOwners.Count == 0 && (___agent.prisoner <= 0 || ___agent.curTileData.chunkID == agent5.curTileData.chunkID) && (!(agent5.agentName == "Cannibal") || !agent5.KnockedOut()) && !agent5.arrested && !agent5.invisible && !__instance.gc.tileInfo.DifferentLockdownZones(___agent.curTileData, agent5.curTileData) && ___agent.curPosX - 5f < agent5.curPosX && ___agent.curPosX + 5f > agent5.curPosX && ___agent.curPosY - 5f < agent5.curPosY && ___agent.curPosY + 5f > agent5.curPosY && ___agent.movement.HasLOSAgent(agent5) && agent5.fire == null)
													{
														___agent.SetPreviousDefaultGoal(___agent.defaultGoal);
														___agent.SetDefaultGoal("Cannibalize");
														___agent.SetCannibalizingTarget(agent5);
								...
								{
									if ((___agent.agentName == "Thief" || ___agent.HasTrait<RoamBehavior_Pickpocket>()) && 
										!__instance.thiefNoSteal) // Pickpocket
									{
									...
									else if (___agent.agentName == "Vampire")
									{
										___agent.losCheckAtIntervalsTime = 0;
										if (!___agent.hasEmployer || ___agent.health <= 15f)
										{
						...
						if (num10 > 17f && num10 != 10000f)
						{
							...
							if (___agent.slaveOwners.Count > 0)
							{
								...
								else
								{
									...
									{
								"Slave Helmet Explosion 1: ",
								___agent,
								" - ",
								___agent.slaveOwners[0]
									}));
